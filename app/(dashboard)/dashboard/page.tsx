...
export default function DashboardPage() {
  ...
  const handleTemplateSelect = (ids: string[]) => {
    ...
    // Auto-advance to generate step on first selection
  ...
  // Generate all selected models from scratch
  const handleGenerate = async () => {
    if (!uploadedFile || selectedTemplates.length === 0) return

    // Reset results to 'generating' for all selected models
    setGenerationResults(
  ...
  const handleRetryFailed = async () => {
    ...
    // Mark failed ones as 'generating' again (keep successful ones as-is)
    setGenerationResults((prev) =>
      prev.map((r) =>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header
        ...
        subtitle="Загрузите украшение, выберите модели и получите лайфстайл-фото"
        ...
        {MOBILE_STEPS.map((step) => {
          ...
          return (
            ...
            >
              ...
              {step.label}
              {/* Show model count badge on step 2 */}
